...
const MOCK_BASE = [
  { folio: 'SOL-2025-001', sectorDependencia: 'STPS / Secretaría del Trabajo', responsable: 'Juan Pérez',    contacto: 'juan.perez@sonora.gob.mx',    estatus: 'En proceso'  },
  { folio: 'SOL-2025-002', sectorDependencia: 'ISSSTESON / Salud',              responsable: 'María López',   contacto: 'maria.lopez@isssteson.gob.mx', estatus: 'Completado'  },
  { folio: 'SOL-2025-003', sectorDependencia: 'SEDESSON / Desarrollo Social',   responsable: 'Carlos Ruiz',   contacto: 'carlos.ruiz@sedesson.gob.mx',  estatus: 'Pendiente'   },
];

const MOCK_SERVIDOR: Reporte12Fila[] = MOCK_BASE.map((r, i) => ({
  ...r,
  ipServidor:          `10.0.${i}.${10 + i}`,
  adminServidor:       `Admin Infraestructura ${i + 1}`,
...
export class ReportesService {
  ...
  getReporte11(filtros: FiltrosReporte): Observable<Reporte11Fila[]> {
    ...
    return this.http.get<Reporte11Fila[]>(`${this.base}/solicitudes-dependencia`, { params: filtros as any });
  }

  getReporte12(filtros: FiltrosReporte): Observable<Reporte12Fila[]> {
    if (environment.useMock) return of(MOCK_SERVIDOR);
    return this.http.get<Reporte12Fila[]>(`${this.base}/recursos-totalizados`, { params: filtros as any });
  }
  ...
  getReporte13(ip: string, filtros: FiltrosReporte): Observable<Reporte13Fila[]> {
    if (environment.useMock) {
      return of(MOCK_SERVIDOR.map(r => ({
        ...r,
        subdominioAprobado: `app-${r.folio.toLowerCase()}.sonora.gob.mx`,
        vpns: 'VPN-2025-001',
      })));
    }
    return this.http.get<Reporte13Fila[]>(`${this.base}/por-ip`, { params: { ip, ...filtros } as any });
  }
  ...
  getReporte21(filtros: FiltrosReporte): Observable<Reporte21Fila[]> {
    ...
    return this.http.get<Reporte21Fila[]>(`${this.base}/vpn`, { params: filtros as any });
  }
  ...
  getReporte22(filtros: FiltrosReporte): Observable<Reporte22Fila[]> {
    ...
    return this.http.get<Reporte22Fila[]>(`${this.base}/subdominios`, { params: filtros as any });
  }
  ...
  getReporte31(filtros: FiltrosReporte): Observable<Reporte31Fila[]> {
    ...
    return this.http.get<Reporte31Fila[]>(`${this.base}/vulnerabilidades`, { params: filtros as any });
  }
  ...
  getReporte32(ip: string, filtros: FiltrosReporte): Observable<Reporte32Fila[]> {
    ...
    return this.http.get<Reporte32Fila[]>(`${this.base}/comunicaciones-ip`, { params: { ip, ...filtros } as any });
  }
  ...
  getReporte41(filtros: FiltrosReporte): Observable<Reporte41Fila[]> {
    ...
    return this.http.get<Reporte41Fila[]>(`${this.base}/estatus-solicitudes`, { params: filtros as any });
  }
  ...
  getReporte42(filtros: FiltrosReporte): Observable<Reporte42Fila[]> {
    if (environment.useMock) {
      return of(MOCK_SERVIDOR.map(r => ({
        ...r,
        subdominios: `app-${r.folio.toLowerCase()}.sonora.gob.mx`,
        vpns: 'VPN-2025-001',
      })));
    }
    return this.http.get<Reporte42Fila[]>(`${this.base}/recursos-general`, { params: filtros as any });
  }
}
